...
if (isset($_POST['save'])) {
    ...
    $mysqli->query("INSERT INTO `oyo_rider_list`(position, name, km_covered, longitude, lat) VALUES('$position', '$name', '$kmCovered', '$longitude', '$lat')") or
        die($mysqli->error);

    $_SESSION['message'] = "Record added successfully";
    $_SESSION['msg_type'] = "success";

    header('Location: oyo.php');
}

if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $mysqli->query("DELETE FROM oyo_rider_list WHERE id=$id") or die(mysqli_error($mysqli));
...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $result = $mysqli->query("SELECT * FROM `oyo_rider_list` WHERE id=$id") or die(mysqli_error($mysqli));
...
if (isset($_POST['update'])) {
    ...
    $mysqli->query("UPDATE oyo_rider_list SET position = '$position', name = '$name', km_covered = '$kmCovered', longitude = '$longitude', lat = '$lat'  WHERE id='$id'") or die(mysqli_error($mysqli));
...
<body>
    ...
    <div class="container">
        <div class="jumbotron">
            <h1 class="sec__info h2"> Update Oyo Leader Board</h1>
        </div>
        <div class="row"><a href="index.php" type="button" class="btn btn-dark">Go Back to LeaderBoard</a></div>
        <br>
        <?php
        $result = $mysqli->query("SELECT * FROM oyo_rider_list") or die($mysqli->error);
        // pre_r($result);
        ?>
            ...
            <table class="table">
                ...
                <?php while ($row = $result->fetch_assoc()) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="oyo.php?edit=<?php echo $row['id']; ?>" class="btn btn-info">Edit</a>
                            <a href="oyo.php?delete=<?php echo $row['id']; ?>" class="btn btn-danger">Delete</a>
