...
class Login
{
    ...
    public function verify_login($post)
    {
        ...
        $user = $this->user_exists($post['phone']);
    ...
    public function register($post)
    {
        ...
        //     return array('status' => 0, 'message' => 'Phone number is not valid');
        // }


        // Check if username exists already
        if (false !== $this->user_exists($post['phone'])) {
            return array('status' => 0, 'message' => 'Phone number already exists');
        }

        if (preg_match('/^([a-z0-9\+\_\-\.]+)@([a-z0-9\+\_\-\.]{2,})(\.[a-z]{2,4})$/i', $post['email']) == false) {
            return array('status' => 0, 'message' => 'Email address is not valid');
        }
    ...
    private function user_exists($where_value, $where_field = 'phone')
    {
